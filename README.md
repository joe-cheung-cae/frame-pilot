...
- JPEG, PNG, WebP, HEIC/HEIF, AVIF, and RAW still imports (`.dng`, `.arw`, `.cr3`, `.nef`). Originals are copied unchanged; thumbnails and previews are WebP; scoring and grouping run on decoded RGB (HEIC/HEIF via local `pillow-heif`, AVIF via Pillow’s native `AvifImagePlugin`, RAW via LibRaw `extract_thumb` when an embedded preview exists, otherwise leftover fallback demosaic). RAW that cannot be developed is skipped with an explicit local message.
...
- RAW files such as DNG, ARW, CR3, and NEF import with an embedded preview (`extract_thumb`) or leftover fallback demosaic when the preview is missing (copy original bytes; WebP from that RGB). FramePilot is not a RAW editor. Files that fail both thumb and demosaic are skipped with an explicit local message. Extra extensions (`.cr2`, `.raf`, `.orf`, `.rw2`) and color-managed develop remain unscheduled. HEIC/HEIF and AVIF stills import locally; Live Photo `.mov` companions and HDR gain-map tone mapping are not implemented. Optional XMP sidecars are export-directory only (never beside originals or in image bytes).
...
Installable Windows (NSIS) and macOS (DMG) builds start the UI and a local API sidecar for you. Current packages are **unsigned**. Prefer the [unsigned GitHub Release](https://github.com/joe-cheung-cae/frame-pilot/releases) (`v2.1.3-desktop`) and the [Unsigned desktop install tutorial](docs/desktop_install.md) for where to download, Gatekeeper / SmartScreen, and start/stop. After install, the [Desktop User Guide](docs/desktop_user_guide.md) covers data directory, path import (copies not moves), and export reveal. Contributor day-to-day work can stay on `npm run dev` (web + API above). Desktop shell development uses `npm run dev:desktop` (requires Rust); see [apps/desktop/README.md](apps/desktop/README.md).

Typical workflow:

1. Create a project.
2. Import JPEG, PNG, WebP, HEIC/HEIF, AVIF, or RAW stills (`.dng`, `.arw`, `.cr3`, `.nef`). Valid files are registered locally, preview generation continues through a visible import job, and a running import can be cancelled at safe checkpoints without deleting originals or completed previews. Same-file reimports or import retries can reuse existing local records and generated previews. RAW that cannot be developed is skipped.
...
CI is signing-ready: Windows Authenticode and macOS Developer ID + notarization run when the full GitHub Actions secret set is present. Missing secrets keep the **unsigned** NSIS/DMG upload green for internal testing. The leftover unsigned GitHub Release (`v2.1.3-desktop`, includes the #198 locked-file hooks, the #195 Import/Export fix, and the #191 sidecar ready-line fix) is still unsigned. Do not use `v2.1.2-desktop` to verify upgrade-while-running. Do not use `v2.1.1-desktop` to verify new-project Import/Export. Do not use `v2.1.0-desktop` for a Win11 cold first Start. Expect SmartScreen / Gatekeeper warnings; do not treat them as a signed store listing. Install walkthrough: [Unsigned desktop install tutorial](docs/desktop_install.md). See [Desktop Code Signing Runbook](docs/desktop_signing.md) and the [Desktop User Guide](docs/desktop_user_guide.md). Missing certificates must not block the first desktop RC. Manual checks: [Desktop Testing Matrix](docs/desktop_testing.md). Phase 9 leftover desktop-plan 2.2 items shipped except auto-download/install; leftover cache knobs shipped ([#175](https://github.com/joe-cheung-cae/frame-pilot/issues/175)); dual-platform installer GUI DoD **install+run** is claimed (Windows NSIS [#144](https://github.com/joe-cheung-cae/frame-pilot/issues/144); leftover macOS DMG [#177](https://github.com/joe-cheung-cae/frame-pilot/issues/177), `2026-09-07T09:34:57Z`). Leftover packaged-desktop ≥500 GUI is claimed ([#179](https://github.com/joe-cheung-cae/frame-pilot/issues/179); packaged WebView from-paths + culling preview, **native dialog stubbed**). Leftover packaged macOS quit+job matrix is claimed ([#181](https://github.com/joe-cheung-cae/frame-pilot/issues/181), `2026-09-08T13:26:18Z`, [desktop.yml run 34230112750](https://github.com/joe-cheung-cae/frame-pilot/actions/runs/34230112750)). Leftover packaged Windows quit+job matrix is claimed ([#184](https://github.com/joe-cheung-cae/frame-pilot/issues/184), `2026-09-08T15:29:33Z`, [desktop.yml run 34242430942](https://github.com/joe-cheung-cae/frame-pilot/actions/runs/34242430942)). See [Known Limitations](docs/v2_known_limitations.md).
