...
def test_zip_selected_files_stores_avif_without_deflate(tmp_path):
    originals = tmp_path / "originals"
    originals.mkdir()
    source = originals / "still.avif"
    source.write_bytes(b"\x00\x00\x00 ftypavif" + b"0" * 1000)
    target = tmp_path / "out.zip"
    zip_selected_files(
        target,
        [{"filename": "still.avif", "original_path": str(source), "project_copy_path": str(source)}],
        project_root=tmp_path,
    )
    with zipfile.ZipFile(target) as archive:
        info = archive.getinfo("still.avif")
        assert info.compress_type == zipfile.ZIP_STORED
