...
On the next launch, leftover import/processing jobs become `interrupted` by default and are reclaimed automatically (Phase 6.1, [#105](https://github.com/joe-cheung-cae/frame-pilot/issues/105)). Leftover exports still fail-and-cleanup and are not reclaimed. Set `FRAMEPILOT_JOB_RECLAIM_ON_STARTUP=0` in the environment (inherited by the sidecar when set) to instead mark leftover import/processing jobs **failed** for manual retry. Original photos are never modified. See [Phase 6 plan](../../docs/plans/2026-08-29-phase6-durable-jobs.md).
