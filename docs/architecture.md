...
The import page polls the returned job id after the response, so long imports show derivative progress and do not leave the user waiting on one opaque request. It also loads the latest import job history so stale, failed, `complete_with_errors`, and `cancelled` import jobs remain visible after navigation or page reload. If an active import job has not updated for more than 30 minutes, the jobs endpoints mark it failed as a stale interrupted local import. This is still a local in-process background task, not a durable external queue: derivative work is not guaranteed to survive API process exits until a durable worker is added.
...
If a queued or running processing job has not updated for more than 30 minutes, the project and jobs endpoints treat it as interrupted, mark it failed, clear any partial groups, remove photo group assignments, reset processed or in-progress photos to retryable imported state, and set the project processed count back to zero. A later process request can then start a replacement job and rebuild groups cleanly.
...
SQLite is currently used as a single-user local database with standard SQLModel engine settings. The app does not currently enable SQLite WAL mode; if WAL is added later for local read/write concurrency, it should be documented and covered by migration or initialization tests.
