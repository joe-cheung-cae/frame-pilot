...
Processing cancellation uses the same `POST /api/projects/{project_id}/jobs/{job_id}/cancel` route and is cooperative, not a hard process kill. Cancelled processing clears groups and returns in-flight photos to `imported`; originals, import derivatives, `user_status`, and `star_rating` stay. Reclaim honors a pending cancel and does not re-queue. Re-run grouping with `POST /process`; `/retry` remains import-only. Export jobs stay uncancellable (`422`).
...
Update 2026-09-03 (Phase 7, [#145](https://github.com/joe-cheung-cae/frame-pilot/issues/145) / [#146](https://github.com/joe-cheung-cae/frame-pilot/issues/146)): the same cancel route cooperatively cancels processing jobs as well as imports. Cancelled processing clears groups; originals stay unchanged. Export cancel remains 422. Pause/resume of in-flight grouping is not implemented.
