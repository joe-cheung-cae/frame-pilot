...
The import page polls the returned job id after the response, so long imports show derivative progress and do not leave the user waiting on one opaque request. If an active import job has not updated for more than 30 minutes, the jobs endpoints mark it failed as a stale interrupted local import. This is still a local in-process background task, not a durable external queue: derivative work is not guaranteed to survive API process exits until a durable worker is added.
