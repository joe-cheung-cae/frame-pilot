...
Import is split into a synchronous upload/register phase and an in-process background derivative phase. The request receives selected local files, validates supported extensions (JPEG, PNG, WebP, HEIC, HEIF, AVIF, and RAW `.dng` / `.arw` / `.cr3` / `.nef`), copies accepted files into the project `originals/` folder, records file identity metadata, creates or reuses `Photo` records, creates a `ProcessingJob` with `job_type` set to `import`, and returns while new photos are still marked `processing`. A FastAPI `BackgroundTasks` worker then opens a fresh database session, generates thumbnails and previews, extracts metadata, computes quality scores, perceptual hashes, and lightweight embeddings, updates per-photo state, and completes the import job as `complete`, `complete_with_errors`, `failed`, or `cancelled`. HEIC/HEIF stills are decoded with local `pillow-heif` (`register_heif_opener`); AVIF stills use Pillow’s native `AvifImagePlugin`; RAW uses `open_raw_import_image` on the copied original (`extract_thumb` first; leftover locked `postprocess` only when thumb fails); derivatives stay WebP; scoring and grouping use decoded still RGB or RAW preview/fallback RGB. Export copies the original HEIC/HEIF/AVIF/RAW bytes. RAW that fails both thumb and demosaic is skipped with an explicit local message and is not copied.

Import derivative generation defaults to one worker. Settings may raise `import_workers` to 2–4 (`GET`/`PATCH /api/settings`, stored in `{data_dir}/app_settings.json`). `n==1` keeps the sequential per-photo loop; `n=2–4` uses an in-process `ThreadPoolExecutor` with one SQLAlchemy session per task. Peak concurrent `process_registered_import_photo` is ≤ n. Cancel stays cooperative (stop scheduling, wait in-flight, no thread kill). Register/copy stays sequential. Processing stays one job per project. There is no Redis, Celery, or extra OS worker process.

Settings **Derivative cache** (`GET /api/cache`) measures files in `{project}/thumbnails/` and `{project}/previews/` only. `POST /api/cache/clear-derivatives` deletes those files, clears `thumbnail_path` / `preview_path`, and leaves originals, review status, ratings, groups, exports, and `{project}/cache/` untouched. A blocking import or processing job returns `409`. The next import or process regenerates missing derivatives.

Desktop Settings can change the FramePilot **app data directory** (`POST /api/desktop/data-dir` after D2.00 `POST /api/desktop/project-roots`). The current data-dir tree is copied into an empty authorized folder; stored `root_path` / photo copy / derivative / export paths whose prefix is the old data dir are rewritten in the destination database only. The old tree stays on disk. `Project.source_root_path` and camera-card originals are never rewritten, copied, moved, or chmod'd. Custom D2.00 project folders outside the old data dir stay put. The desktop shell persists `{anchor}/data_dir.json` (after `FRAMEPILOT_DATA_DIR`, before the default app-support / `.framepilot-desktop-dev` path) and respawns the sidecar with the new `--data-dir`. No extra `fs:` / `shell:` capabilities.

Desktop Help → **Check for updates** (S9.10 / [#167](https://github.com/joe-cheung-cae/frame-pilot/issues/167)) is a native menu action. A helper thread performs an unauthenticated `GET` of GitHub Releases latest; the WebView CSP stays loopback-only. The Releases JSON is the manifest. Missing `tag_name` is a non-fatal no-op. The shell compares MAJOR.MINOR.PATCH cores (`2.1.0-desktop` vs `v2.2.0`) and shows a local dialog. There is no launch-time check, auto-install, `tauri-plugin-updater`, extra `fs:` / `shell:` capabilities, telemetry, or GitHub token.
...
Update 2026-09-05 (Phase 9 S9.08, [#168](https://github.com/joe-cheung-cae/frame-pilot/issues/168)): Settings may raise import **derivative** workers from 1 to 4. The value is snapshotted at `run_import_derivative_job` start. Processing remains one job per project.
...
Optional XMP sidecars (`include_xmp`, default off) are derived export artifacts only. Folder copies get `{exported_filename}.xmp` beside the image; ZIP archives include the same member names. CSV accepts and stores the flag but writes no `.xmp` files. Sidecars never land in `originals/`, beside camera originals, or inside image bytes.
