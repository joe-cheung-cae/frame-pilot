...
POST   /api/desktop/menu-command
GET    /api/desktop/menu-command
...
`POST /api/desktop/menu-command` and `GET /api/desktop/menu-command` exist only when `FRAMEPILOT_DESKTOP=1`; otherwise they return `404`. `POST` accepts `{"command": "import"}` for a navigable File menu command (`new`, `shortcuts`, `import`, `export`, `process`, `cull`) and returns `204`. Other ids return `422`. `GET` takes the oldest queued command and returns `{"command": "import"}` or `{"command": null}` with `Cache-Control: no-store`. The packaged Win11 shell posts on File menu click and polls this GET so navigation does not depend on Tauri `eval` / `invoke`. The queue is in-memory, FIFO, and capped at 16.
