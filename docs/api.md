...
When `POST /api/projects` includes a custom `root_path`, it must point to a usable local directory location. Invalid storage paths return `422` before project metadata is created.
