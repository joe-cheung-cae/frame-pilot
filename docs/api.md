...
`active_import_job` is the newest non-stale queued or running import job for the project, or `null` when import derivative work is not active. The project list and dashboard use this lightweight field to route active-import projects back to import progress instead of processing or culling. `GET /api/projects` is read-only: it omits stale jobs from `active_import_job` without writing. Stale job failure writes happen on project detail, jobs endpoints, mutations, and API startup instead.
...
This background path improves request responsiveness and visible progress. Startup reclaim is on by default (Phase 6.1, [#105](https://github.com/joe-cheung-cae/frame-pilot/issues/105)): leftover active import/processing jobs are marked `interrupted` and reclaimed in-process (or via `npm run worker` / `python -m app.worker`) instead of failing outright. Set `FRAMEPILOT_JOB_RECLAIM_ON_STARTUP=0` (or `false`/`no`/`off`) to opt back into the legacy behavior, where leftover active jobs are marked failed on the next startup so the user can retry. Export jobs still fail-and-cleanup on restart regardless of the flag. See [Phase 6 plan](plans/2026-08-29-phase6-durable-jobs.md).
...
- `interrupted`: reclaimable leftover after an API/sidecar restart, unless startup reclaim was explicitly disabled; not a successful terminal state and not active work.
...
Supported still formats are JPEG, PNG, WebP, HEIC, HEIF, AVIF (`.avif` stills only, not `.avifs` sequences), and RAW (`.dng`, `.arw`, `.cr3`, `.nef`). HEIC/HEIF files are copied into `originals/` unchanged, decoded locally with `pillow-heif`, and given WebP thumbnails/previews. AVIF stills use the same copy-and-WebP-derivative path, decoded with Pillow’s native `AvifImagePlugin` (not the HEIF opener). RAW files are copied unchanged; FramePilot uses `open_raw_import_image` (`rawpy.extract_thumb` first; leftover [#202](https://github.com/joe-cheung-cae/frame-pilot/issues/202) locked `postprocess` only when thumb fails) and builds WebP derivatives from that RGB. FramePilot is not a RAW editor. Scoring and grouping use the decoded still RGB or RAW preview/fallback RGB. ZIP and folder export ship the original HEIC/HEIF/AVIF/RAW bytes (`ZIP_STORED`). RAW that fails both thumb and demosaic is skipped with `RAW file could not be developed; no embedded preview and demosaic failed` and is not copied into `originals/`. Garbage HEIC or AVIF bytes fail that file after copy instead of being treated as an unsupported extension.
