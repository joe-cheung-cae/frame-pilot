...
Supported still formats are JPEG, PNG, WebP, HEIC, HEIF, and AVIF (`.avif` stills only, not `.avifs` sequences). HEIC/HEIF files are copied into `originals/` unchanged, decoded locally with `pillow-heif`, and given WebP thumbnails/previews. AVIF stills use the same copy-and-WebP-derivative path, decoded with Pillow’s native `AvifImagePlugin` (not the HEIF opener). Scoring and grouping use that decoded RGB. ZIP and folder export ship the original HEIC/HEIF/AVIF bytes (`ZIP_STORED`). RAW extensions such as `.dng`, `.arw`, `.cr3`, and `.nef` are still skipped with an explicit unsupported-format reason. Garbage HEIC or AVIF bytes fail that file after copy instead of being treated as an unsupported extension.
