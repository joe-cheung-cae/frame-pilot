...
来自 `desktop` 工作流的未签名 Windows / macOS 产物仅供**内部测试者与开发者**使用，不是已签名公开商店上架。残留未签名 GitHub Release（`v2.1.3-desktop`，含 #198 锁文件 hooks、#195 导入/导出修与 #191 sidecar ready-line 修）是持久下载入口；它仍然**未签名**。验应用仍在运行时升级不要用 `v2.1.2-desktop`。验新建工程导入/导出不要用 `v2.1.1-desktop`。Win11 冷首启不要用 `v2.1.0-desktop`。

逐步下载、Gatekeeper / SmartScreen、启动与退出见 [未签名桌面安装教程](desktop_install.zh.md)。

测试者应当：

- 从未签名 GitHub Release 或项目的 GitHub Actions `desktop` 运行下载安装包，不要从第三方镜像下载
...
- `.github/workflows/desktop-release.yml` — 从这些产物发布未签名 GitHub Release（`v2.1.3-desktop`）
...
- [docs/desktop_unsigned_release_notes.md](desktop_unsigned_release_notes.zh.md) — 未签名 Release 正文
