...
`POST /api/desktop/menu-command` 和 `GET /api/desktop/menu-command` 仅在 `FRAMEPILOT_DESKTOP=1` 时存在；否则返回 `404`。`POST` 接受可跳页的 File 菜单命令 `{"command": "import"}`（`new`、`shortcuts`、`import`、`export`、`process`、`cull`），返回 `204`。其他 id 返回 `422`。`GET` 取出队列最旧的一条，返回 `{"command": "import"}` 或 `{"command": null}`，并带 `Cache-Control: no-store`。包装 Win11 壳在 File 菜单点击时 POST，再 poll 这个 GET，跳页不依赖 Tauri `eval` / `invoke`。队列在内存里，FIFO，最多 16 条。
