...
**`main` 上的下一步：** 第九阶段剩余 stretch 收口（S9.00–S9.13），每次运行一个 GitHub issue。S9.00 与 S9.01 已落地；从 S9.02 开始。不要发明第十阶段。下文关于作业持久性、桌面打包、处理取消和 HEIC 预览的条目描述的是 2026-06-05 rc2 产品，除非另有后续切片说明。见 `develop_plan.zh.md` §1.1 与 `docs/plans/2026-09-04-remaining-stretch.zh.md`。
