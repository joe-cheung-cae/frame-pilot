...
Unsigned Windows and macOS artifacts from the `desktop` workflow are intended for **internal testers and developers only**, not for a signed public store listing. The leftover unsigned GitHub Release (`v2.1.3-desktop`, includes the #198 locked-file hooks, the #195 Import/Export fix, and the #191 sidecar ready-line fix) is the durable download; it is still **unsigned**. Do not use `v2.1.2-desktop` to verify upgrade-while-running. Do not use `v2.1.1-desktop` to verify new-project Import/Export. Do not use `v2.1.0-desktop` for a Win11 cold first Start.

Step-by-step download, Gatekeeper / SmartScreen, start, and quit: [Unsigned desktop install tutorial](desktop_install.md).

Testers should:

- Download installers from the unsigned GitHub Release or the project’s GitHub Actions `desktop` runs — not third-party mirrors
...
- `.github/workflows/desktop-release.yml` — publishes the unsigned GitHub Release (`v2.1.3-desktop`) from those artifacts
...
- [docs/desktop_unsigned_release_notes.md](desktop_unsigned_release_notes.md) — unsigned Release body
