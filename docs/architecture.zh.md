...
导入分为同步的上传/登记阶段和进程内后台衍生阶段。请求接收所选本地文件，校验受支持的扩展名（JPEG、PNG、WebP、HEIC、HEIF、AVIF，以及 RAW `.dng` / `.arw` / `.cr3` / `.nef`），将接受的文件复制到项目的 `originals/` 文件夹，记录文件身份元数据，创建或复用 `Photo` 记录，创建 `job_type` 为 `import` 的 `ProcessingJob`，并在新照片仍标记为 `processing` 时返回。随后 FastAPI `BackgroundTasks` 工作器打开新的数据库会话，生成缩略图和预览，提取元数据，计算质量分数、感知哈希和轻量嵌入，更新每张照片的状态，并将导入作业完成状态设为 `complete`、`complete_with_errors`、`failed` 或 `cancelled`。HEIC/HEIF 静帧用本地 `pillow-heif`（`register_heif_opener`）解码；AVIF 静帧用 Pillow 自带的 `AvifImagePlugin`；RAW 对已拷原片调用 `open_raw_import_image`（先 `extract_thumb`；仅当 thumb 失败才走残留锁定 `postprocess`）；衍生件仍是 WebP；评分和分组使用解码后的静帧 RGB 或 RAW 预览/回退 RGB。导出拷贝原始 HEIC/HEIF/AVIF/RAW 字节。thumb 与 demosaic 都失败的 RAW 以明确本地消息跳过，不会拷贝。
