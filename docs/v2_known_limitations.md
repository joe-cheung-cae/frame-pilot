...
- CI is **signing-ready**: Authenticode / Developer ID + notarization run when the full GitHub Actions secret set is present. Missing secrets keep the **unsigned** upload green. The leftover unsigned GitHub Release (`v2.1.0-desktop`) is still unsigned. See [Desktop Code Signing Runbook](desktop_signing.md).
