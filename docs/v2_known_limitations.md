...
- RAW (`.dng`, `.arw`, `.cr3`, `.nef`; copy original bytes; LibRaw `extract_thumb` when an embedded preview exists, otherwise leftover [#202](https://github.com/joe-cheung-cae/frame-pilot/issues/202) locked `postprocess` fallback; WebP derivatives from that RGB; original bytes exported)

Unsupported files are reported locally instead of uploaded or decoded remotely.

## Deferred Formats

Color-managed RAW develop and extra RAW extensions remain deferred (unscheduled). Leftover [#202](https://github.com/joe-cheung-cae/frame-pilot/issues/202) ships fallback LibRaw `postprocess` when `extract_thumb` fails (`use_camera_wb=True`, `no_auto_bright=True`, `output_bps=8`, `half_size=True`). FramePilot is not a RAW editor: no exposure / white-balance UI. `extract_raw_preview_image` stays thumb-only. Files that fail both thumb and demosaic are skipped with `RAW file could not be developed; no embedded preview and demosaic failed` and are not copied into `originals/`. Extra RAW extensions such as `.cr2`, `.raf`, `.orf`, and `.rw2` are not accepted. HEIC/HEIF and AVIF stills import locally; Live Photo `.mov` companions, `.avifs` sequences, and HDR/gain-map tone mapping are not implemented. XMP is not written on import, into `originals/`, beside camera files, or into image bytes.
...
- HEIC/HEIF stills and RAW (`.dng`, `.arw`, `.cr3`, `.nef`) import locally (same as the web app). Embedded preview uses `extract_thumb`; leftover [#202](https://github.com/joe-cheung-cae/frame-pilot/issues/202) fallback demosaic when no preview exists. Both-path failure is skipped with a local message.
...
- CI is **signing-ready**: Authenticode / Developer ID + notarization run when the full GitHub Actions secret set is present. Missing secrets keep the **unsigned** upload green. The leftover unsigned GitHub Release (`v2.1.3-desktop`, includes [#199](https://github.com/joe-cheung-cae/frame-pilot/pull/199) / [#198](https://github.com/joe-cheung-cae/frame-pilot/issues/198) locked-file hooks, [#195](https://github.com/joe-cheung-cae/frame-pilot/pull/195) Import/Export, and [#191](https://github.com/joe-cheung-cae/frame-pilot/pull/191) sidecar ready-line / 120s) is still unsigned. Do not use `v2.1.2-desktop` to verify upgrade-while-running. Do not use `v2.1.1-desktop` to verify new-project Import/Export. Do not use `v2.1.0-desktop` for a Win11 cold first Start. See [Desktop Code Signing Runbook](desktop_signing.md).
- Leftover desktop-plan 2.2 items shipped in Phase 9 (tray S9.06, detached preview S9.07, import workers S9.08, data-dir S9.09, check-for-updates S9.10) and leftover cache knobs ([#175](https://github.com/joe-cheung-cae/frame-pilot/issues/175)), except auto-download/install. Dual-platform installer GUI DoD **install+run** is claimed ([#177](https://github.com/joe-cheung-cae/frame-pilot/issues/177), `2026-09-07T09:34:57Z`). Leftover packaged-desktop ≥500 GUI is claimed ([#179](https://github.com/joe-cheung-cae/frame-pilot/issues/179); packaged WebView from-paths + culling preview, **native dialog stubbed**). Leftover packaged macOS quit+job matrix is claimed ([#181](https://github.com/joe-cheung-cae/frame-pilot/issues/181), `2026-09-08T13:26:18Z`, [desktop.yml run 34230112750](https://github.com/joe-cheung-cae/frame-pilot/actions/runs/34230112750)). Leftover packaged Windows quit+job matrix is claimed ([#184](https://github.com/joe-cheung-cae/frame-pilot/issues/184), `2026-09-08T15:29:33Z`, [desktop.yml run 34242430942](https://github.com/joe-cheung-cae/frame-pilot/actions/runs/34242430942)). Leftover Windows packaged sidecar first-launch ready-line timeout is fixed in code ([#190](https://github.com/joe-cheung-cae/frame-pilot/issues/190); 120s Windows budget + `sidecar.ready`); leftover NSIS locked `_internal` upgrade is fixed in installer hooks ([#198](https://github.com/joe-cheung-cae/frame-pilot/issues/198); Retry/Cancel, no Ignore-through). Use leftover [#200](https://github.com/joe-cheung-cae/frame-pilot/issues/200) [`v2.1.3-desktop`](https://github.com/joe-cheung-cae/frame-pilot/releases/tag/v2.1.3-desktop) (`2026-09-09T09:01:01Z`) to verify upgrade-while-running. Published `v2.1.2-desktop` NSIS does not include those hooks — quit FramePilot before upgrading that build, and never Ignore file-in-use errors. Do not invent a new dated Windows GUI pass here.
- **Packaged macOS DMG GUI lifecycle S9.12 is skip, not pass** ([#172](https://github.com/joe-cheung-cae/frame-pilot/issues/172), `2026-09-05T12:31:10Z`) and stays history. Leftover install+run DoD passed on Darwin `macos-latest` (`2026-09-07T09:34:57Z`, [desktop.yml run 34105891421](https://github.com/joe-cheung-cae/frame-pilot/actions/runs/34105891421), `APP_VERSION=2.1.0-desktop`). Windows NSIS GUI lifecycle is recorded on [#144](https://github.com/joe-cheung-cae/frame-pilot/issues/144) (Windows-only). Unsigned Gatekeeper warnings remain expected. See [Desktop Testing Matrix](desktop_testing.md).
...
- Settings **Derivative cache** shows thumbnail and preview size (`GET /api/cache`). **Clear thumbnails and previews** deletes only `{project}/thumbnails/` and `{project}/previews/` (`POST /api/cache/clear-derivatives`) and returns `409` while an import or processing job is queued, running, or interrupted. Originals, picks, ratings, groups, exports, and `{project}/cache/` stay. The next import or process regenerates missing derivatives.
...
Unsigned install walkthrough: [Unsigned desktop install tutorial](desktop_install.md) (prefer the unsigned GitHub Release). After install: [Desktop User Guide](desktop_user_guide.md).
