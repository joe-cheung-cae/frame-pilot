...
- CI is **signing-ready**: Authenticode / Developer ID + notarization run when the full GitHub Actions secret set is present. Missing secrets keep the **unsigned** upload green. The leftover unsigned GitHub Release (`v2.1.1-desktop`, includes [#191](https://github.com/joe-cheung-cae/frame-pilot/pull/191) sidecar ready-line / 120s) is still unsigned. Do not use `v2.1.0-desktop` for a Win11 cold first Start. See [Desktop Code Signing Runbook](desktop_signing.md).
