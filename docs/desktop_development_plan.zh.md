...
> **文档版本**：1.17  
> **创建日期**：2026-08-18  
> **最近审阅**：2026-09-11（残留 #202 需求拆解：无内嵌预览时的 RAW 回退显影）  
...
- [x] Windows 与 macOS 均可通过标准安装包安装并运行 — Windows NSIS GUI 为 pass（[#144](https://github.com/joe-cheung-cae/frame-pilot/issues/144)）；残留 macOS DMG 安装并运行（[#177](https://github.com/joe-cheung-cae/frame-pilot/issues/177)，`2026-09-07T09:34:57Z`，[desktop.yml run 34105891421](https://github.com/joe-cheung-cae/frame-pilot/actions/runs/34105891421)）。S9.12 skip（[#172](https://github.com/joe-cheung-cae/frame-pilot/issues/172)）作为历史保留。残留退出+作业矩阵见 [#181](https://github.com/joe-cheung-cae/frame-pilot/issues/181)（不要重勾本安装并运行行）。
...
第九阶段已交付的 2.2 残留改指向对应 S9 id。真正剩下的继续延后（未排期）。不要发明第十阶段 / 2.3。
...
| RAW 回退显影（无内嵌预览） | 残留 [#202](https://github.com/joe-cheung-cae/frame-pilot/issues/202) 仅在 `extract_thumb` 失败时 LibRaw `postprocess`；不是 RAW 编辑器；S9.04 [#162](https://github.com/joe-cheung-cae/frame-pilot/issues/162) 仍只抽 thumb | 残留 [#202](https://github.com/joe-cheung-cae/frame-pilot/issues/202) `[ ]` |
...
- **无 GUI 主机（如 WSL2）**：sidecar 可启动、`/health` 正常、收到 SIGTERM 后退出；路径导入不修改原文件；`npm run test:api` 与 `npm run verify` 通过；Tauri 相关任务标记为 blocked-gui 并在 [docs/desktop_feasibility_notes.md](desktop_feasibility_notes.zh.md) 记录命令与错误。
...
| 残留 #202 | 无内嵌预览时的 RAW 回退 demosaic（不是第十阶段）。计划：[docs/plans/2026-09-11-leftover-raw-develop.md](plans/2026-09-11-leftover-raw-develop.zh.md) |
| 未排期 | 自动下载安装、处理池、色彩管理 RAW / 额外扩展名、SmartScreen/商店上架。不要发明第十阶段 / 2.3 |

发布渠道建议：

- GitHub Releases（主渠道）— 残留 [#200](https://github.com/joe-cheung-cae/frame-pilot/issues/200) 发布含 #198 锁文件 hooks、#195 导入/导出修以及此前 #191 sidecar ready-line 修的**未签名** `v2.1.3-desktop` NSIS + DMG。残留 [#196](https://github.com/joe-cheung-cae/frame-pilot/issues/196) 曾发布 `v2.1.2-desktop`（#198 hooks 之前）。残留 [#192](https://github.com/joe-cheung-cae/frame-pilot/issues/192) 曾发布 `v2.1.1-desktop`（#195 之前）。残留 [#188](https://github.com/joe-cheung-cae/frame-pilot/issues/188) 曾发布 `v2.1.0-desktop`（sidecar 修之前）。不是 Gatekeeper 干净 / SmartScreen 干净 / 商店上架。
- 后续可考虑官网下载页
...
- [docs/plans/2026-08-18-desktop-packaging.md](plans/2026-08-18-desktop-packaging.zh.md) — 按 D0.00…D5.05 逐条执行的详细开发计划（含 §5.1 任务勾选表；2026-08-18 审阅结论已并入此文件）
- [docs/desktop_goal_mode.md](desktop_goal_mode.zh.md) — Grok Build / Codex Goal Mode 可粘贴提示词
...
- [README.md](../README.zh.md) — 当前功能与运行方式
- [develop_plan.md](../develop_plan.zh.md) — v2 总规划（含桌面 stretch goal）
- [docs/plans/2026-08-18-desktop-packaging.md](plans/2026-08-18-desktop-packaging.zh.md) — 桌面化逐任务实施计划（Goal Mode backlog；技术冲突以此为准；2026-08-18 审阅结论已并入此文件，原审阅文件已删除）
- [docs/desktop_goal_mode.md](desktop_goal_mode.zh.md) — 桌面化 Goal Mode 提示词
- [docs/v2_architecture.md](v2_architecture.zh.md) — 架构（desktop packaging 在 v2.0 中仍为 deferred；桌面轨道单独推进）
- [docs/v2_product_requirements.md](v2_product_requirements.zh.md) — 产品边界
...
- [docs/desktop_install.md](desktop_install.zh.md) — 未签名 Windows NSIS / macOS DMG 安装教程（Release 下载、Gatekeeper / SmartScreen、启动/退出）
- [docs/desktop_unsigned_release_notes.md](desktop_unsigned_release_notes.zh.md) — 未签名 GitHub Release 正文
