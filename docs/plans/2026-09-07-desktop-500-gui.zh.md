...
- [ ] 开发 — Slice 2 harness+python、Slice 3 QA 门闩、Slice 4 `desktop.yml`+awk 一起做（**不要**勾 §2.2）。Slice 1 Linux skip-not-pass 主机检查已在树里；仅在 Slice 4 之后才勾残留计划 开发 `[x]`。
- [ ] 测试
- [ ] 上线 — 在本分支 dispatch `desktop.yml`；两个 OS 的 500 都 `result=pass`，stamp 文档，再以 stub 文案勾 §2.2

本次 归档 提交只勾 **归档**。开发 保持 `[ ]`。不要勾 §2.2。开发 必须落地 Slice 2、Slice 3 与 Slice 4；两个 OS 的 500 都 `result=pass` 之前 **不得**勾 §2.2。
