...
`apps/desktop/src/lib/desktopQaRunner.ts` 挂在 `App.tsx` 里，作为 **`MemoryRouter` 的子节点**，**不**在 `isPreviewWindow()` 分支。包装后的 Tauri 自定义协议 URL 是 `/index.html`；`BrowserRouter` / `HashRouter` / `MemoryRouter` 的 `navigate` 在 WebView 里不会重挂 `CullingWorkspace`（GHA 34143375303 / 34145603768 / 34146923344）。Path B 用 store（`useSyncExternalStore`）挂 `CullingWorkspace`，不走 router。cull href 放在 `window` 上。`QaOrRoutes` 用 `useSyncExternalStore` 加 100ms 轮询读取，事件漏了也能挂上 `CullingWorkspace`。
