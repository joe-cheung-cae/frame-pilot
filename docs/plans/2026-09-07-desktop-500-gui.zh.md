...
`apps/desktop/src/lib/desktopQaRunner.ts` 挂在 `App.tsx` 里，作为 **`HashRouter` 的子节点**，**不**在 `isPreviewWindow()` 分支。包装后的 Tauri 自定义协议 URL 是 `/index.html`；`BrowserRouter` 的 `navigate('/projects/:id/cull')` 挂不上 `CullingWorkspace`。
