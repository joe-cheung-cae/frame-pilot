...
`apps/desktop/src/lib/desktopQaRunner.ts` 挂在 `App.tsx` 里，作为 **`MemoryRouter` 的子节点**，**不**在 `isPreviewWindow()` 分支。包装后的 Tauri 自定义协议 URL 是 `/index.html`；`BrowserRouter` / `HashRouter` / `MemoryRouter` 的 `navigate` 在 WebView 里不会重挂 `CullingWorkspace`（GHA 34143375303 / 34145603768 / 34146923344）。`useSyncExternalStore` / window 轮询也没让 `QaOrRoutes` 重挂（GHA 34150390831）。Path B 在 `CullOverlay` 渲染时把 React `setState` 注册到 `window`（和 `navigate` 同一套路），再从 `waitForCullPush` 调用，从而不走 router 也能挂上 `CullingWorkspace`。
