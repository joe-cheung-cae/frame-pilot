...
- [x] 上线 — [desktop.yml run 34155284835](https://github.com/joe-cheung-cae/frame-pilot/actions/runs/34155284835) 两个 OS 的 500 都 `result=pass`；stamp 文档；以 stub 文案勾 §2.2

测试 勾残留计划 **测试**。上线 oracle 见 §17（[desktop.yml run 34125376154](https://github.com/joe-cheung-cae/frame-pilot/actions/runs/34125376154)，未声称 DoD）。双边 500 绿灯见 §19。
...
`qa_write_evidence(line: String)`：参数 **只有** `line`。目标路径是启动时缓存的 evidence 路径。校验为带字符串 `milestone` 与 `t` 的单个 JSON 对象；拒绝 U+000A / U+000D。QA 启用时 `milestone` 必须在白名单内。追加并 flush。在 `capabilities/qa.json` 里只给 `main` 窗口显式 ACL `allow-qa-write-evidence`（**不要**依赖 `core:default`；**不要**授予 `preview`）。QA 标志为假时拒绝。QA 启用时对 env 路径 `canonicalize`，再确认仍是 scratch 前缀下的严格子孙。

`spawn_sidecar` `env_remove` `FRAMEPILOT_DESKTOP_QA*`。继续 `env_remove` `FRAMEPILOT_PROJECT_ROOT_ALLOWLIST`。

### 7.2 SPA

`apps/desktop/src/lib/desktopQaRunner.ts` 挂在 `App.tsx` 里，作为 **`MemoryRouter` 的子节点**，**不**在 `isPreviewWindow()` 分支。包装后的 Tauri 自定义协议 URL 是 `/index.html`；`BrowserRouter` / `HashRouter` / `MemoryRouter` 的 `navigate` 在 WebView 里不会重挂 `CullingWorkspace`（GHA 34143375303 / 34145603768 / 34146923344）。window 上的 `setState` 被调用了（`via=mount`），但活着的 App 树没有重挂（GHA 34152454215，没有 `cull_workspace`）。Path B 用 `createRoot` 把 `CullingWorkspace` 挂到 document overlay，并用 `listPhotos` + `assetUrl` 插入 `/previews/` 的 `<img>`。
...
第十一次 `workflow_dispatch`（`5fbb133`）：[34148113854](https://github.com/joe-cheung-cae/frame-pilot/actions/runs/34148113854)（`2026-09-07T17:45:20Z`，`conclusion=failure`）。两边 `cull_push` 都是 `via=store` 且立刻带上 `cull_project_id`，`route=/`，fail `imgs=0`。store 写在 Path B 那个模块里；`QaOrRoutes` 没重挂（`desktopQaRunner` 被打成两份）。500 skip。**未声称 DoD。** §2.2 保持 `[ ]`。后续：把 cull href 放到 `window` 上，并用 window 事件通知。

第十二次 `workflow_dispatch`（`5f1e7b2`）：[34149252152](https://github.com/joe-cheung-cae/frame-pilot/actions/runs/34149252152)（`2026-09-07T18:04:33Z`，`conclusion=failure`）。两边 `cull_push` 都是立刻 `via=store`，`route=/`，fail `imgs=0`。Path B 写了 window；`QaOrRoutes` 仍没重挂（事件没接到 / 没有轮询）。500 skip。**未声称 DoD。** §2.2 保持 `[ ]`。后续：`QaOrRoutes` 用 `useSyncExternalStore` + 100ms 轮询读 `window`。

第十三次 `workflow_dispatch`（`1634c82`）：[34150390831](https://github.com/joe-cheung-cae/frame-pilot/actions/runs/34150390831)（`2026-09-07T18:20:15Z`，`conclusion=failure`）。两边都是立刻 `cull_push` `via=store`，`route=/`，fail `imgs=0`。`QaOrRoutes` 里 100ms 的 `useSyncExternalStore` 轮询仍没挂上 `CullingWorkspace`（sidecar 仍是 `GET /api/projects` + `jobs?limit=50`；没有 `GET /photos`）。500 skip。**未声称 DoD。** §2.2 保持 `[ ]`。后续：渲染时把 React `setState` 注册到 `window`，由 Path B 调用（`via=mount`）。

第十四次 `workflow_dispatch`（`7ac2dfe`）：[34152454215](https://github.com/joe-cheung-cae/frame-pilot/actions/runs/34152454215)（`2026-09-07T18:51:58Z`，`conclusion=failure`）。两边都是立刻 `cull_push` `via=mount`，没有 `cull_workspace`，fail `imgs=0`。window 上的 `setState` 跑了；活着的 `CullOverlay` 没重渲染（sidecar 仍是 `GET /api/projects` + `jobs?limit=50`）。500 skip。**未声称 DoD。** §2.2 保持 `[ ]`。后续：`createRoot` overlay + 从 `listPhotos` 插入 `/previews/` `<img>`。

第十五次 `workflow_dispatch`（`bc2e397`）：[34153819674](https://github.com/joe-cheung-cae/frame-pilot/actions/runs/34153819674)（`2026-09-07T19:13:01Z`，`conclusion=failure`）。Path B overlay **管用了**：两边 probe 都是 `result=pass`（`cull_workspace` `via=root`，`preview_natural_width=1800`）；macOS 500 `result=pass`（`accepted_files=500`）。Windows 500 没跑起来：`prepare_scratch` `rm -rf` probe 留下的 `framepilot.db` 失败（`Device or resource busy`；probe 的 GUI 还在）。**未声称 DoD。** §2.2 保持 `[ ]`。后续：500 前先杀掉残留进程并重试 wipe。

---

## 19. 上线 oracle（2026-09-07T19:39:38Z）

第十六次 `workflow_dispatch`（`b2fe1ff`）：[34155284835](https://github.com/joe-cheung-cae/frame-pilot/actions/runs/34155284835)（`created_at` `2026-09-07T19:21:46Z`，`conclusion=success`）。草稿 PR [#180](https://github.com/joe-cheung-cae/frame-pilot/pull/180) 在上线提交里改为 `Fixes` [#179](https://github.com/joe-cheung-cae/frame-pilot/issues/179)。**声称 DoD**：残留包装桌面 ≥500 GUI，包装 WebView from-paths + 审片预览（**原生对话框 stub**）。不是完整 S9.12 点击走查。不改 `APP_VERSION`。未改 `verify.yml`。Issue [#144](https://github.com/joe-cheung-cae/frame-pilot/issues/144) / [#172](https://github.com/joe-cheung-cae/frame-pilot/issues/172) / [#177](https://github.com/joe-cheung-cae/frame-pilot/issues/177) 保持关闭。

| OS / 步骤 | 结论 |
| --------- | ---- |
| Windows probe | `result=pass`（同 job 画图；不是 §2.2 勾选） |
| Windows 500 | `result=pass`，`2026-09-07T19:39:23Z`。产物 [FramePilot-desktop-500-gui-500-windows](https://github.com/joe-cheung-cae/frame-pilot/actions/runs/34155284835/artifacts/10031111392)：`mode=500` `accepted_files=500` `native_dialog=stubbed` `originals_unchanged=true` `app_version=2.1.0-desktop` `preview_natural_width=1800` sidecar 峰值 213.05 MB UI 峰值 317.56 MB |
| macOS #177 安装并运行 | 同 job `result=pass`。不是本残留 DoD。 |
| macOS probe | `result=pass`（同 job 画图；不是 §2.2 勾选） |
| macOS 500 | `result=pass`，`2026-09-07T19:30:33Z`。产物 [FramePilot-desktop-500-gui-500-macos](https://github.com/joe-cheung-cae/frame-pilot/actions/runs/34155284835/artifacts/10030923306)：`mode=500` `accepted_files=500` `native_dialog=stubbed` `originals_unchanged=true` `app_version=2.1.0-desktop` `preview_natural_width=1800` sidecar 峰值 566.66 MB UI 峰值 568.92 MB |

双平台 500：**`result=pass`**。勾残留计划 上线 `[x]`。以 stub 文案勾 `docs/desktop_development_plan.md` §2.2 ≥500。Linux `--probe` 仍是 exit 2 / skip 不是 pass。
