...
需求拆解（Slice 0）是文档合同。Slice 1 Linux skip-not-pass 已在树里。本 评审 提交**不要**扩展 harness，也不要加 `desktop.yml` 的 probe/500 步骤。**不要**勾 开发、DoD-ticked 或 §2.2。
...
| 文件 | 需求拆解 | 开发 | 上线 |
| ---- | -------- | ---- | ---- |
| `docs/plans/2026-09-07-desktop-500-gui.md`（+ zh） | 创建 | Slice 4 之后勾 开发 | 勾 上线 |
| `.grok/workflows/desktop-500-gui.rhai` | 若存在则纳入 | 不变 | 不变 |
| `packaging/scripts/desktop-500-gui.sh` | **不做**（Slice 1 stub 已在树里） | Slice 2 扩展 | 不变 |
| `packaging/scripts/stamp-desktop-500-gui-docs.py` | 不做 | 创建 | 对 500 JSON 运行 |
| `tests/desktop/desktop-500-gui-linux.sh` | 不做（Slice 1 已创建） | 保持绿 | 不变 |
