...
11. **Project roots on desktop:** `FRAMEPILOT_PROJECT_ROOT_ALLOWLIST` stays a deployment-level control and must NEVER be set to `$HOME`, `/`, a drive root, or any broad parent by the Tauri shell. Sidecar spawn `env_remove`s that variable so a parent shell cannot leak a wide allowlist. A user-chosen project root becomes legal only after D2.00 registration persisted in `{data_dir}/desktop_project_roots.json`. `test_create_project_rejects_root_outside_allowlist` must stay green unchanged.
...
- [x] `npm run verify` green — 2026-08-28T17:28:00+08:00 (rust-free; D3.01 GUI `[x]` 2026-08-31; D3.02 GUI `[x]` 2026-08-31; D3.03 GUI `[x]` 2026-08-31)
