...
导入与处理工作使用本地 API 进程中的 FastAPI `BackgroundTasks`。任务具有可见进度、过期检测和重试路径，但**默认**不能在 API 进程退出后持久存活。如果 API 进程在工作期间停止，之后的轮询会将过期的排队或运行中任务标记为失败，以便用户在可能时重试。

**可选回收（第六阶段）：** 设置 `FRAMEPILOT_JOB_RECLAIM_ON_STARTUP=1` 后，启动时会将残留的活跃导入/处理任务标为 `interrupted`，并在进程内自动续跑导入衍生件（以及对中断的处理任务清理不完整分组后重建）。也可通过 `npm run worker` / `python -m app.worker` 运行本地 worker 入口。未设置该开关时默认仍为失败并重试。导出任务在重启时仍失败并清理。详见 [第六阶段计划](plans/2026-08-29-phase6-durable-jobs.zh.md)。
...
桌面端在导入仍活动时关闭，可以 POST 同一取消路由，最多等待 10 秒，然后对 sidecar 发送 SIGTERM。处理任务仍活动时关闭不能取消它（`POST .../cancel` 返回 422）；选择仍然退出会对 sidecar 发送 SIGTERM，下次启动时启动扫描会将该任务标记为 `failed`。硬杀死不会被标记为 `cancelled`。处理任务仍然没有取消路由。

## 重试语义

导入重试适用于失败、`complete_with_errors`、过期失败以及已取消的导入任务。重试会创建新的导入任务，保留现有 Photo ID、`user_status` 和 `star_rating`，复用有效衍生件，并在可能时从本地已复制原图重新生成缺失衍生件。重试不会让任务在 API 重启后变得持久，也不会重新注册一个新的外部源文件夹。
