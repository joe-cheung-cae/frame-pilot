...
- CI 已**签名就绪**：完整 GitHub Actions secret 集在场时会做 Authenticode / Developer ID + 公证。缺少 secrets 时保持**未签名**上传绿灯。残留未签名 GitHub Release（`v2.1.2-desktop`，含 [#195](https://github.com/joe-cheung-cae/frame-pilot/pull/195) 导入/导出与 [#191](https://github.com/joe-cheung-cae/frame-pilot/pull/191) sidecar ready-line / 120 秒）仍然未签名。Win11 冷首启不要用 `v2.1.0-desktop`。验新建工程导入/导出不要用 `v2.1.1-desktop`。见 [桌面代码签名手册](desktop_signing.zh.md)。
