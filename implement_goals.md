...
- Phase 9 remaining-stretch is closed (S9.00–S9.13). Leftover cache knobs (#175) shipped. Leftover dual-platform installer GUI DoD install+run (#177) shipped. Leftover packaged-desktop ≥500 GUI (#179) shipped. Leftover packaged macOS quit+job matrix (#181) shipped. Leftover packaged Windows quit+job matrix (#184) shipped. Leftover unsigned install tutorial (#186) shipped. Leftover unsigned desktop GitHub Release (#188) shipped. Leftover unsigned desktop Release with Import/Export fix (#196) shipped. Leftover Win11 NSIS locked `_internal` upgrade (#198) is the current leftover. If §1.1 names no next slice, do not start a new implementation loop.
